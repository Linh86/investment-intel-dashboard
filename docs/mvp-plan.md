...
| M1 | SQLite/Drizzle/Zod, raw-item ingestion, deterministic DEMO_MODE triage, `POST /api/runs/morning-brief`, real run history | Run button creates run, step, and signal rows offline with no API keys |
| M2 | Risk scorer, score history, planted export-control event, provenance panel | Planted event moves a score; rationale cites it; every number traces to sources |
